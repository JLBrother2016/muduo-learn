...
class WeakCallback                       // 模板类, 而且还是可变参数的模板类
{
 public:

  WeakCallback(const boost::weak_ptr<CLASS>& object,
               const std::function<void (CLASS*, ARGS...)>& function)
    : object_(object), function_(function)   // 构造函数
  {
  }

  // Default dtor, copy ctor and assignment are okay

  void operator()(ARGS&&... args) const      // 重载括号()操作符
  {
    boost::shared_ptr<CLASS> ptr(object_.lock());  // 智能指针管理
    if (ptr)
    {
