...
// C 库宏 offsetof(type, member-designator) 会生成一个类型为 size_t 
// 的整型常量，它是一个结构成员相对于结构开头的字节偏移量
BOOST_STATIC_ASSERT(offsetof(sockaddr_in, sin_family) == 0);// 检查字节偏移是否一样
...
InetAddress::InetAddress(uint16_t port, bool loopbackOnly, bool ipv6)
{
  ...
  if (ipv6)    // 是否是ipv6的地址
  {
    ...
    in6_addr ip = loopbackOnly ? in6addr_loopback : in6addr_any;  // 回环地址
    addr6_.sin6_addr = ip;
...
bool InetAddress::resolve(StringArg hostname, InetAddress* out)
{
  ...
  // gethostbyname()函数主要作用：用域名或者主机名获取地址, gethostbyname_r是可重入的
