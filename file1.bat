...
if not exist "%base_path%%file_name%" (
    echo 将需要下载的视频信息放到文件 "%base_path%%file_name%" 里。
    goto bat_end
)
...
for /L %%i in (1 1 %video_num%) do (
    you-get -O "%base_path%%%i" "%download_url%#page=%%i"
)
...
if %check_count% geq 3 (
    echo 下载过程出现错误，没有全部下载。
    goto bat_end
)
C: > "%base_path%check.txt"
for /L %%i in (1 1 %video_num%) do (
...
echo 一共 %line_count% 个视频已经下载到 "%base_path:~0,-1%"
